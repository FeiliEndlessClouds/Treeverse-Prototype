...
public class ObjectPool : MonoBehaviour
{
	...
	// Changed original to delay Start call by one frame mimicking unity 
	// behavior and allowing object to be placed in correct hierarchy 
	...
	public GameObject Instantiate( Vector3 position, Quaternion rotation)
	{
		...
		else // else pull one from the cache
		{
			...
			StartCoroutine(DoDelayedStart(obj));
		}
		
		return obj;
	}

	IEnumerator DoDelayedStart(GameObject obj)
	{
		yield return null;
		obj.SetActive(true);
		obj.SendMessage( "Start", SendMessageOptions.DontRequireReceiver );
